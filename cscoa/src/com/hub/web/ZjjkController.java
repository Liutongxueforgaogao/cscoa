...
public class ZjjkController {
	...
	private String auditPage(Map<String, Object> map,HttpServletRequest request){
		...
		try { 
			...
			List<AuditPage> allAudit = zjjkAuditPageServiceImpl.getAllAudit(username, pagenum);
			map.put("audits", allAudit);
			map.put("pagenum", pagestr);
			//计算未审核的个数
			int auditSize = zjjkAuditPageServiceImpl.getAuditSize(username);
	...
	private String detailsPage(HttpServletRequest request,Map<String, Object> map){
		...
		//取所有流程中人的建议
		ArrayList<UserAndSuggest> allPeopleSuggest = zjjkAuditQueryServiceImpl.getAllPeopleSuggest(checkid);
		...
		String status = zjjkAuditQueryServiceImpl.getStatus(username, checkid);
		//将审核状态传给前端判断
		if(status==null){
...
/*		try {
	...
	@RequestMapping("/searchByConditionZjjk")
...
//			pdzt = (String) jb.fromObject(str).get("pdkwv");
		...
		List<AuditPage> allAudit=zjjkAuditPageServiceImpl.getAuditBycondition(username, pddh, pdrqstart,pdrqend, pdzt);
		int size = allAudit.size();
		map.put("zjjk_size",size);
...
//		}
	...
	public String disagree(Map<String, Object> map,HttpServletRequest request){
		...
		if(username!=null&&!username.equals("")&&status==null){
			try {
				zjjkAuditQueryServiceImpl.insertSuggest(checkid, username, replyMessage,replyMessage1);
				zjjkAuditQueryServiceImpl.disagree(username, checkid);
			} catch (Exception e) {
				map.put("error", "打回失败");
			}	
			//status="0";
		...
		map.put("allPeopleSuggest", allPeopleSuggest);
		//获取审核状态
		status = zjjkAuditQueryServiceImpl.getStatus(username, checkid);
		//将审核状态传给前端判断
		if(status==null){
			map.put("atag", "0");//表示未审核			
		}else if(status.equals("1")){
	...
	public String agree(Map<String, Object> map,HttpServletRequest request){
		...
		if(username!=null&&status==null){
			try {
				zjjkAuditQueryServiceImpl.insertSuggest(checkid, username, replyMessage,replyMessage1);	
				zjjkAuditQueryServiceImpl.agree(username, checkid);
			} catch (Exception e) {
				map.put("error", "同意失败");
			}
			//status="1";
