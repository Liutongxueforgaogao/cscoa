...
public class SdjkController {
	...
	private String auditPage(Map<String, Object> map,HttpServletRequest request){
		...
		try {
			...
			List<AuditPage> allAudit = sdjkAuditPageServiceImpl.getAllAudit(username, pagenum);
			map.put("audits", allAudit);
			map.put("pagenum", pagestr);
			//计算未审核的个数
			int auditSize = sdjkAuditPageServiceImpl.getAuditSize(username);
	...
	private String detailsPage(HttpServletRequest request,Map<String, Object> map){
		...
		//取所有流程中人的建议
		ArrayList<UserAndSuggest> allPeopleSuggest = sdjkAuditQueryServiceImpl.getAllPeopleSuggest(checkid);
		...
		String status = sdjkAuditQueryServiceImpl.getStatus(username, checkid);
		//将审核状态传给前端判断
		if(status==null){
...
/*		try {
	...
	@RequestMapping("/searchByConditionSdjk")
...
//			pdzt = (String) jb.fromObject(str).get("pdkwv");
		...
		List<AuditPage> allAudit=sdjkAuditPageServiceImpl.getAuditBycondition(username, pddh, pdrqstart,pdrqend, pdzt);
		int size = allAudit.size();
		map.put("sdjk_size",size);
...
//		}
	...
	public String disagree(Map<String, Object> map,HttpServletRequest request){
		...
		if(username!=null&&!username.equals("")&&status==null){
			try {
				sdjkAuditQueryServiceImpl.insertSuggest(checkid, username, replyMessage,replyMessage1);
				sdjkAuditQueryServiceImpl.disagree(username, checkid);
			} catch (Exception e) {
				map.put("error", "打回失败");
			}	
			//status="0";
		...
		map.put("allPeopleSuggest", allPeopleSuggest);
		//获取审核状态
		status = sdjkAuditQueryServiceImpl.getStatus(username, checkid);
		//将审核状态传给前端判断
		if(status==null){
			map.put("atag", "0");//表示未审核			
		}else if(status.equals("1")){
	...
	public String agree(Map<String, Object> map,HttpServletRequest request){
		...
		if(username!=null&&status==null){
			try {
				sdjkAuditQueryServiceImpl.insertSuggest(checkid, username, replyMessage,replyMessage1);	
				sdjkAuditQueryServiceImpl.agree(username, checkid);
			} catch (Exception e) {
				map.put("error", "同意失败");
			}
			//status="1";
