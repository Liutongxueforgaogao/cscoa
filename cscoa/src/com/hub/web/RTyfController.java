...
public class RTyfController {
	...
	private String auditPage(Map<String, Object> map,HttpServletRequest request){
		...
		try {
			...
			List<AuditPage> allAudit = rTyfAuditPageServiceImpl.getAllAudit(username, pagenum);
			map.put("audits", allAudit);
			map.put("pagenum", pagestr);
			//计算未审核的个数
			int auditSize = rTyfAuditPageServiceImpl.getAuditSize(username);
	...
	private String detailsPage(HttpServletRequest request,Map<String, Object> map){
		...
		if(checkid!=""){
			...
			for (Rtm00101 rtm00101item : allInformationByRtm00101) {
				...
				System.out.println("看这里1："+expensename);
		...
		//取所有流程中人的建议
		ArrayList<UserAndSuggest> allPeopleSuggest = rTyfAuditQueryServiceImpl.getAllPeopleSuggest(checkid);
		...
		String status = rTyfAuditQueryServiceImpl.getStatus(username, checkid);
		//将审核状态传给前端判断
		if(status==null){
...
/*		try {
	...
	@RequestMapping("/searchByConditionRTYF")
...
//			pdzt = (String) jb.fromObject(str).get("pdkwv");
		...
		List<AuditPage> allAudit=rTyfAuditPageServiceImpl.getAuditBycondition(username, pddh, pdrqstart,pdrqend, pdzt);
		int size = allAudit.size();
		map.put("rtyf_size",size);
...
//		}
	...
	public String disagree(Map<String, Object> map,HttpServletRequest request){
		...
		if(username!=null&&!username.equals("")&&status==null){
			try {
				rTyfAuditQueryServiceImpl.insertSuggest(checkid, username, replyMessage,replyMessage1);
				rTyfAuditQueryServiceImpl.disagree(username, checkid);
			} catch (Exception e) {
				map.put("error", "打回失败");
			}	
			//status="0";
		...
		map.put("allPeopleSuggest", allPeopleSuggest);
		//获取审核状态
		status = rTyfAuditQueryServiceImpl.getStatus(username, checkid);
		//将审核状态传给前端判断
		if(status==null){
			map.put("atag", "0");//表示未审核			
		}else if(status.equals("1")){
	...
	public String agree(Map<String, Object> map,HttpServletRequest request){
		...
		if(username!=null&&status==null){
			try {
				rTyfAuditQueryServiceImpl.insertSuggest(checkid, username, replyMessage,replyMessage1);	
				rTyfAuditQueryServiceImpl.agree(username, checkid);
			} catch (Exception e) {
				map.put("error", "同意失败");
			}
			//status="1";
