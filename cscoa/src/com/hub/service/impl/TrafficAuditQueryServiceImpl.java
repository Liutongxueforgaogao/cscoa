...
@Service("TrafficAuditQueryServiceImpl")
public class TrafficAuditQueryServiceImpl {

	@Autowired(required = false) 
	private TrafficAuditQueryDao trafficAuditQueryDao;
	...
	public ArrayList<UserAndSuggest> getAllPeopleSuggest(String systemcode){
		ArrayList<UserAndSuggest> allPeopleSuggest = trafficAuditQueryDao.getAllPeopleSuggestTraffic(systemcode);
...
//					output.add(userSuggest);
	...
	public void insertSuggest(String checkid,String shr,String message,String message1) {
		...
		trafficAuditQueryDao.insertSuggestTraffic(checkid,date,shr,message, message1);
	}
	
	public AuditLevelQuery getAuditLevelQuery(String stepremarks,
			String systemcode) {
		AuditLevelQuery auditLevelQuery = trafficAuditQueryDao.getAuditLevelQueryTraffic(stepremarks, systemcode);
	...
	public Suggestion getSuggest(String checkid, String systemcode) {
		...
		AuditLevelQuery auditLevelQuery = trafficAuditQueryDao.getAuditLevelQueryTraffic(checkid, systemcode);
	...
	public void disagree(String userLevel, String systemcode) {
		trafficAuditQueryDao.disagreeTraffic(userLevel, systemcode);
	}

	public void agree(String userLevel, String systemcode) {
		trafficAuditQueryDao.agreeTraffic(userLevel, systemcode);
	}

	public String getStatus(String username, String systemcode) {
		String status = trafficAuditQueryDao.getStatusTraffic(username, systemcode);
		return status;
	}
