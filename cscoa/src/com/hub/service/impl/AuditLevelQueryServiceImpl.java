...
@Service("AuditLevelQueryServiceImpl")
public class AuditLevelQueryServiceImpl implements AuditLevelQueryService {

	@Autowired(required = false) 
	private AuditLevelQueryDao auditLevelQueryDao;
	...
	public ArrayList<UserAndSuggest> getAllPeopleSuggest(String systemcode){
		ArrayList<UserAndSuggest> allPeopleSuggest = auditLevelQueryDao.getAllPeopleSuggest(systemcode);
...
//					output.add(userSuggest);
	...
	public void insertSuggest(String checkid,String shr,String message,String message1) {
		...
		auditLevelQueryDao.insertSuggest(checkid,date,shr,message, message1);
	}
	
	public AuditLevelQuery getAuditLevelQuery(String stepremarks,
			String systemcode) {
		AuditLevelQuery auditLevelQuery = auditLevelQueryDao.getAuditLevelQuery(stepremarks, systemcode);
	...
	public Suggestion getSuggest(String checkid, String systemcode) {
		...
		AuditLevelQuery auditLevelQuery = auditLevelQueryDao.getAuditLevelQuery(checkid, systemcode);
	...
	public void disagree(String userLevel, String systemcode) {
		auditLevelQueryDao.disagree(userLevel, systemcode);
	}

	
	public void agree(String userLevel, String systemcode) {
		auditLevelQueryDao.agree(userLevel, systemcode);
	}


	
	public String getStatus(String username, String systemcode) {
		String status = auditLevelQueryDao.getStatus(username, systemcode);
		return status;
	}
