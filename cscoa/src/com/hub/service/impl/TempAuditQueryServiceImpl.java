...
@Service("TempAuditQueryServiceImpl")
public class TempAuditQueryServiceImpl {

	@Autowired(required = false) 
	private TempAuditQueryDao tempAuditQueryDao;
	...
	public ArrayList<UserAndSuggest> getAllPeopleSuggest(String systemcode){
		ArrayList<UserAndSuggest> allPeopleSuggest = tempAuditQueryDao.getAllPeopleSuggestTemp(systemcode);
...
//					output.add(userSuggest);
	...
	public void insertSuggest(String checkid,String shr,String message,String message1) {
		...
		tempAuditQueryDao.insertSuggestTemp(checkid,date,shr,message, message1);
	}
	
	public AuditLevelQuery getAuditLevelQuery(String stepremarks,
			String systemcode) {
		AuditLevelQuery auditLevelQuery = tempAuditQueryDao.getAuditLevelQueryTemp(stepremarks, systemcode);
	...
	public Suggestion getSuggest(String checkid, String systemcode) {
		...
		AuditLevelQuery auditLevelQuery = tempAuditQueryDao.getAuditLevelQueryTemp(checkid, systemcode);
	...
	public void disagree(String userLevel, String systemcode) {
		tempAuditQueryDao.disagreeTemp(userLevel, systemcode);
	}

	public void agree(String userLevel, String systemcode) {
		tempAuditQueryDao.agreeTemp(userLevel, systemcode);
	}

	public String getStatus(String username, String systemcode) {
		String status = tempAuditQueryDao.getStatusTemp(username, systemcode);
		return status;
	}
