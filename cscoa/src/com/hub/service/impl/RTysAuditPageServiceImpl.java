...
@Service("RTysAuditPageServiceImpl")
public class RTysAuditPageServiceImpl implements AuditPageService {

	@Autowired(required = false) 
	private RTyanshouAuditPageDao rTyanshouAuditPageDao;
	
	public List<AuditPage> getAllAudit(String username,int pagenum) {
		List<AuditPage> allAudit = rTyanshouAuditPageDao.getAllAudit(username,pagenum);
		return allAudit;
	}	
	 public int getAuditSize(String username){
		 int auditSize = rTyanshouAuditPageDao.getAuditSize(username);
		 return auditSize;
	 }
	 //获取多部门未审核个数
	...
	public List<AuditPage> getAuditBycondition(String username, String pddh, String pdrqstart,String pdrqend, String pdzt) {
		...
		List<AuditPage> auditResult = rTyanshouAuditPageDao.getAuditBycondition(username, pddh, pdrqstart,pdrqend, pdzt);
		return auditResult;
	}
