...
@Service("TravelAuditQueryServiceImpl")
public class TravelAuditQueryServiceImpl {

	@Autowired(required = false) 
	private TravelAuditQueryDao travelAuditQueryDao;
	...
	public ArrayList<UserAndSuggest> getAllPeopleSuggest(String systemcode){
		ArrayList<UserAndSuggest> allPeopleSuggest = travelAuditQueryDao.getAllPeopleSuggestTravel(systemcode);
...
//					output.add(userSuggest);
	...
	public void insertSuggest(String checkid,String shr,String message,String message1) {
		...
		travelAuditQueryDao.insertSuggestTravel(checkid,date,shr,message, message1);
	}
	
	public AuditLevelQuery getAuditLevelQuery(String stepremarks,
			String systemcode) {
		AuditLevelQuery auditLevelQuery = travelAuditQueryDao.getAuditLevelQueryTravel(stepremarks, systemcode);
	...
	public Suggestion getSuggest(String checkid, String systemcode) {
		...
		AuditLevelQuery auditLevelQuery = travelAuditQueryDao.getAuditLevelQueryTravel(checkid, systemcode);
	...
	public void disagree(String userLevel, String systemcode) {
		travelAuditQueryDao.disagreeTravel(userLevel, systemcode);
	}

	public void agree(String userLevel, String systemcode) {
		travelAuditQueryDao.agreeTravel(userLevel, systemcode);
	}

	public String getStatus(String username, String systemcode) {
		String status = travelAuditQueryDao.getStatusTravel(username, systemcode);
		return status;
	}
