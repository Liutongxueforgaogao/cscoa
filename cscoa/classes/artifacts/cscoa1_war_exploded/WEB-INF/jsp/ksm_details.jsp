...
<body>
	<div data-role="page" id="page2">
		...
		<div data-role="header" data-position="fixed">
			<a href="KsmAudit?pagenum=0" data-ajax="false" class="back" data-role="none"
...
<tr>
	<td colspan="2">
		<label style="font-size:16px;color:#bf2020;">
			${details.billtype eq null?"":"单据类型:" }
			<c:if test="${details.billtype=='01' }">付款验收单</c:if>
			<c:if test="${details.billtype=='02' }">财务专用暂扣单</c:if>
			<c:if test="${details.billtype=='03' }">暂付款申请单</c:if>
			<c:if test="${details.billtype=='04' }">交通费</c:if>
			<c:if test="${details.billtype=='05' }">差旅费</c:if>
		</label>
	</td>
</tr>
...
<tr>
	<td colspan="2">
		<label style="font-size:13px;color:#bf2020;">
			${details.revcompanyname eq null?"":"付款对象:" }${details.revcompanyname}
		</label>
	</td>
</tr>
<tr>
	<td colspan="2">
		<label style="font-size:13px;color:#bf2020;">
			${details.totalamount eq null?"":"金额:" }${details.totalamount}
		</label>
	</td>
</tr>
<tr>
	<td colspan="2">
		<label style="font-size:13px;color:#bf2020;">
			${details.bank eq null?"":"银行:" }${details.bank}
		</label>
	</td>
</tr>
<tr>
	<td colspan="2">
		<label style="font-size:13px;color:#bf2020;">
			${details.bankcode eq null?"":"银行账号:" }${details.bankcode}
		</label>
	</td>
</tr>

<c:if test="${(!empty details.prepayno)&&(details.billtype!='03') }">
	<tr>
		<td width="50%">
			<label style="font-size:13px;color:#bf2020;">
	...
	<div data-role="popup" id="popupParis5" data-overlay-theme="b" data-theme="b" data-corners="false">
		...
		<div data-role="header" data-theme="a" class="ui-corner-top">
			...
			<div data-role="content" data-theme="d" class="ui-corner-bottom ui-content">
				...
				<form id="form_1" action="ksm_agree?checkid=${details.checkid }" method="post">
		...
		<div data-role="header" data-theme="a" class="ui-corner-top">
			...
			<div data-role="content" data-theme="d" class="ui-corner-bottom ui-content">
				<form id="form_2" action="ksm_disagree?checkid=${details.checkid }" method="post">
					<textarea cols="30" rows="5" name="replyMessage"></textarea>			
					<div align="center" style="display: none;"><span style="color:red">您没有输入内容!</span></div>
					<br>
					<!-- 取消以及确定按钮，确认按钮点击后则将输入数据保存在数据库中 -->
